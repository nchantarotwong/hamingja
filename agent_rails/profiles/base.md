...
- before reading a large file, run `agent-rails locate "<what you need>"` or a repo-specific semantic navigator, then read only the returned line range. If you do not yet know the right symbol or section, run `agent-rails code-atlas` first to get a file map
- when a file keeps attracting broad reads or repeated edits, run `agent-rails repo-health` and consider whether the file should be split before adding more unrelated code
