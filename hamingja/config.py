...
Trust model (this is the security boundary):
  ...
  * The DEFAULTS below and the packaged config/config.default.json are TRUSTED
    — they ship with hamingja / are set by the operator who installed it.
  * User-level config under ~/.hamingja/ is TRUSTED operator state. It may
  ...
  * A per-project .hamingja.json is read from the agent's CURRENT WORKING
  ...
  * The HAMINGJA_MODE env var is operator-controlled (the human's shell),
    so it IS trusted and may set any valid mode, including "enforce".

Resolution order:
  1. built-in defaults
  2. packaged config.default.json            (trusted)        -> sanitized baseline
  3. ~/.hamingja/config.json               (trusted)        -> may tighten
  4. ~/.hamingja/policies/*.json (matched) (trusted)        -> may tighten
  5. per-project .hamingja.json (cwd)      (untrusted)      -> relax-only overlay
  6. .hamingja-off marker (cwd)            -> mode "off"
  7. HAMINGJA_MODE env var                 (trusted)        -> any valid mode
...
def _restrict_merge(baseline: dict, project) -> dict:
    ...
    if isinstance(pbud, dict):
        ...
        # agent opt into it via `hamingja budget task-type set`. Such a
...
_TRUSTED_HOME_ENV = "HAMINGJA_HOME"


def _search_dirs(start: str):
    """Yield `start` and its ancestors, stopping at a repo/home/filesystem
    boundary. This lets a .hamingja.json / .hamingja-off placed at the
...
def _apply_trusted_config(cfg: dict, start: str) -> dict:
    ...
    Unlike repo-local .hamingja.json, this layer is operator-owned and may
...
def load_config(project_dir: Optional[str] = None) -> dict:
    ...
    try:
        ov = _find_upwards(start, ".hamingja.json")
        if ov is not None:
    ...
    try:
        if _find_upwards(start, ".hamingja-off") is not None:
            baseline["mode"] = "off"
    except Exception:
        pass

    # 7. trusted env override (operator-controlled; may tighten)
    env = _canon_mode(os.environ.get("HAMINGJA_MODE"))
    if env is not None:
        baseline["mode"] = env
