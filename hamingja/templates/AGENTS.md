...
[`hamingja`](https://github.com/nchantarotwong/hamingja) package itself and runs as a hook,
...
This block is managed by `hamingja init`. Anything you write *outside*
the surrounding `<!-- BEGIN/END hamingja workflow profiles -->` markers
is preserved on re-run; anything *inside* is rewritten. Re-run
`hamingja init` to update; pass `--force` to clobber the entire file.
