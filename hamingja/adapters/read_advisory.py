...
def large_read_advisory(tool: str, args: object) -> str | None:
    ...
    return (
        f"[hamingja] {name} has ~{line_count} lines. "
        f"Prefer: `hamingja locate \"<what you need>\"`, then Read only "
        f"the suggested line range with offset+limit. If you need a map first, "
        f"run `hamingja code-atlas`. Unscoped reads of large files are the "
