...
from hamingja.adapters.read_advisory import large_read_advisory, large_read_line_count  # noqa: E402
from hamingja.core.api import check  # noqa: E402
from hamingja.core.budget import BLOCK as BUDGET_BLOCK  # noqa: E402
from hamingja.core.budget import NUDGE as BUDGET_NUDGE  # noqa: E402
from hamingja.core.budget import increment_and_check as budget_check  # noqa: E402
from hamingja.detectors.base import BLOCK, NUDGE  # noqa: E402
from hamingja.ledger import advisory_for_tool as ledger_advisory  # noqa: E402
from hamingja.ledger import discover_root as ledger_root  # noqa: E402
...
def _contains_hamingja_wrapper(command: str) -> str:
    try:
        tokens = [tok.lower() for tok in shlex.split(command)]
    except ValueError:
        return ""
    for i, tok in enumerate(tokens[:-1]):
        if tok.rsplit("/", 1)[-1] != "hamingja":
            continue
        sub = tokens[i + 1]
...
def _is_budget_command(tool: str, tool_input) -> bool:
    """True for Bash calls that are hamingja budget commands.
    ...
    try:
        ...
        return False
    for i, tok in enumerate(tokens[:-1]):
        if tok.rsplit("/", 1)[-1] == "hamingja" and tokens[i + 1] == "budget":
            return True
    return False
...
def _budget_tool_name(tool: str, tool_input) -> str:
    ...
    Recognized hamingja ledger commands get command-family names so the
    ...
    try:
        ...
        for i, tok in enumerate(tokens[:-2]):
            if tok.rsplit("/", 1)[-1] == "hamingja" and tokens[i + 1] == "ledger":
                action = tokens[i + 2]
                if action in {"add", "check", "relevant", "retire", "reverify"}:
                    return f"Bash:hamingja ledger {action}"
                return str(tool)
        return str(tool)
    except Exception:
        return str(tool)
...
def _read_quota_safe(session_id: str):
    ...
    """
    try:
        from hamingja.adapters.codex.quota import read_quota  # noqa: PLC0415
...
def _codex_escalation_context(tool: str, tool_input) -> str:
    ...
    wrapper = _contains_hamingja_wrapper(command)
    if not wrapper:
        return ""
    return (
        "Codex sandbox preflight: this hamingja wrapper usually needs "
...
def main() -> int:
    ...
    try:
        ...
        budget_nudge = ""
        try:
            from hamingja.config import load_config  # noqa: PLC0415
            cfg = load_config(cwd)
            if cfg.get("mode") != "off":
