"""`hamingja` command-line entry point.

A thin operator-facing CLI over the same core the hooks use. Subcommands:

    hamingja report [--reset]   tuning summary: what fired, would-block rates
    hamingja status [DIR]       resolved config for DIR (default: cwd)
    hamingja commands           list workflow wrappers available here
    hamingja preflight [NAME]   list or run repo-owned preflight scripts
    hamingja code-atlas [DIR]   map large files to bounded line ranges
    hamingja repo-health [DIR]  show large-file retrieval cost
    hamingja locate QUERY       ranked code ranges to read next
    hamingja locate-symbol NAME ranked definition-ish ranges
    hamingja locate-edit QUERY  ranked ranges for a desired change
    hamingja ledger ...          manage repo-local ruled-out records
    hamingja budget ID [ACTION] inspect/add/reset session budget
    hamingja install [HARNESS]  install hooks; no arg = all detected harnesses
    hamingja uninstall [HARNESS] remove only hamingja hooks; preserve others
    hamingja init [...]         compose a CLAUDE.md + AGENTS.md symlink from profiles
    hamingja pr-create ...      create a PR with a body file
    hamingja pr-merge PR        wait for CI checks, merge + poll + local cleanup
    hamingja ci-status          summarize PR checks
    hamingja ci-preflight       classify CI quota/infrastructure blocks
    hamingja ci-failures        summarize failed CI logs
    hamingja test-summary LOG   summarize saved pytest output
    hamingja version
...
from .hook_lifecycle import (
    HARNESS_ALIASES as _HARNESS_ALIASES,
    KNOWN_HARNESSES as _KNOWN_HARNESSES,
    detect_harnesses as _detect_harnesses,
    uninstall as _uninstall_hooks,
)
...
def _cmd_report(args: argparse.Namespace) -> int:
    ...
    if not entries:
        ...
        return 0
    s = summarize(entries)
    print(f"hamingja report  ({s['total']} verdicts across {s['sessions']} session(s))")
    print()
...
def _cmd_status(args: argparse.Namespace) -> int:
    ...
    env = os.environ.get("HAMINGJA_MODE")
    if env:
        print(f"\n(HAMINGJA_MODE={env} is set in your shell and overrides mode)")
    return 0


_GLOBAL_WRAPPERS = [
    ("PR", "hamingja pr-create --title <title> --body-file <path>"),
    ("PR", "hamingja pr-create --title <title> --body -"),
    ("PR", "hamingja pr-merge <pr>"),
    ("PR", "hamingja pr-merge <pr> --skip-ci-reason <reason>"),
    ("PR", "hamingja post-merge-cleanup [branch]"),
    ("CI", "hamingja ci-status [pr]"),
    ("CI", "hamingja ci-status [pr] --wait"),
    ("CI", "hamingja ci-preflight [pr]"),
    ("CI", "hamingja ci-failures <pr>"),
    ("CI", "hamingja ci-failures --pr <pr>"),
    ("CI", "hamingja ci-failures --run <run-id>"),
    ("Tests", "hamingja test-summary .pytest_output.log"),
]
...
_PREFLIGHT_DIR = Path(".hamingja") / "preflight"
...
def _discover_repo_root(start: Path) -> Optional[Path]:
    ...
    for candidate in (path, *path.parents):
        if (candidate / ".git").exists() or (candidate / ".hamingja").exists():
            return candidate
    return None
...
def _repo_preflight_commands(root: Path) -> list[tuple[str, str]]:
    ...
    for path in _repo_preflight_scripts(root):
        out.append(("Preflight", f"hamingja preflight {path.name}"))
    return out
...
def _cmd_commands(args: argparse.Namespace) -> int:
    ...
    print("Available hamingja wrappers for this repo:")
...
def _cmd_preflight(args: argparse.Namespace) -> int:
    ...
    env = dict(os.environ)
    env["HAMINGJA_REPO_ROOT"] = str(root)
    env["HAMINGJA_PREFLIGHT_NAME"] = name
    try:
...
def _cmd_uninstall(args: argparse.Namespace) -> int:
    return _uninstall_hooks(args.harness)
...
def _cmd_recover(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        try:
            ...
            latest_recovery = None
        print(f"# hamingja recovery handoff: {session_id}")
        print("")
        print("Last tripwire:")
        if latest_recovery:
        ...
        print(f"- hamingja recover {session_id} reset")
        print(f"- hamingja budget {session_id} reset")
        print(f"- Fresh session: hamingja recover {session_id} handoff")
        return 0
    except Exception:
...
def _cmd_budget(args: argparse.Namespace) -> int:
    ...
    if self_approve and action != "add":
        ...
        print(f"  Use: hamingja budget {session_id} add 3 --self", file=sys.stderr)
        return 2
    if approve_subagent and action not in {"add", "subagent"}:
        ...
        print(f"  Use: hamingja budget {session_id} subagent", file=sys.stderr)
        return 2

    if action == "status":
        if value is not None:
            print("error: status does not take a value", file=sys.stderr)
            print(f"  Use: hamingja budget {session_id}", file=sys.stderr)
            return 2
        state = _budget_read_state(session_id)
        if not state:
    ...
    if action == "add":
        ...
        if add is None:
            ...
            print(f"  Use: hamingja budget {session_id} add 20", file=sys.stderr)
            return 2
        if self_approve:
            ...
            if not result.get("ok"):
                ...
                print(
                    f"  Human approval: ! hamingja budget {session_id} add {add}",
                    file=sys.stderr,
                )
                return 1
        ...
        if not state:
            ...
            print(f"  To clear bad state: hamingja budget {session_id} reset", file=sys.stderr)
            return 1
    ...
    if action == "reset":
        ...
        if add is None:
            ...
            print(f"  Use: hamingja budget {session_id} reset 20", file=sys.stderr)
            return 2
...
def _print_budget_next_steps(session_id: str) -> None:
    print("next:")
    print(f"  hamingja budget {session_id} add 20")
    print(f"  hamingja budget {session_id} reset")
    print(f"  hamingja budget {session_id} reset 20")
...
def _print_budget_examples(*, file=None) -> None:
    ...
    print("  hamingja budget <session-id>", file=file)
    print("  hamingja budget <session-id> add 20", file=file)
    print("  hamingja budget <session-id> reset", file=file)
    print("  hamingja budget <session-id> reset 20", file=file)
    print("  hamingja budget <session-id> subagent", file=file)
    print("  hamingja budget <session-id> add 3 --self", file=file)
...
def _cmd_budget_task_type(args: argparse.Namespace, parts: list[str]) -> int:
    ...
    print("  hamingja budget task-type list", file=sys.stderr)
    print("  hamingja budget task-type get <session-id>", file=sys.stderr)
    print("  hamingja budget task-type set <session-id> <type>", file=sys.stderr)
    return 2
...
# hamingja-managed block within an existing CLAUDE.md / AGENTS.md. The
...
_BEGIN_MARKER = "<!-- BEGIN hamingja workflow profiles -->"
_END_MARKER = "<!-- END hamingja workflow profiles -->"
_MANAGED_BLOCK_RE = re.compile(
    re.escape(_BEGIN_MARKER) + r".*?" + re.escape(_END_MARKER),
    re.DOTALL,
)
_MANAGED_NOTICE = (
    "<!-- managed by `hamingja init`; edits between these markers will be "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--version", action="version", version=f"hamingja {__version__}")
    ...
    pfl.add_argument("name", nargs="?", help="preflight script name under .hamingja/preflight/")
    ...
    unins = sub.add_parser(
        "uninstall",
        help="remove only hamingja hooks while preserving other harness config",
    )
    unins.add_argument(
        "harness", nargs="?",
        choices=["claude", "claude_code", "codex", "all"],
    ...
    sub.add_parser("version", help="print version").set_defaults(
        func=lambda _a: (print(f"hamingja {__version__}"), 0)[1]
    )
    ...
    bgt = sub.add_parser(
        ...
        epilog=(
            "examples:\n"
            "  hamingja budget <session-id>\n"
            "  hamingja budget <session-id> add 20\n"
            "  hamingja budget <session-id> reset\n"
            "  hamingja budget <session-id> reset 20\n"
            "  hamingja budget <session-id> subagent\n"
            "  hamingja budget <session-id> add 3 --self\n"
            "  hamingja budget task-type list\n"
            "  hamingja budget task-type set <session-id> <type>"
        ),
