...
Config keys (from cfg dict, typically cfg["budget"]):
  ...
  weights:         dict {tool_name: float} — per-tool cost multipliers for the
                   ...
                   such as "Bash:hamingja ledger relevant".
...
_DEFAULT_WEIGHTS: dict[str, float] = {
    ...
    "WebFetch": 0.75,
    "WebSearch": 0.75,
    # hamingja ledger bookkeeping. The adapters classify these recognized
    ...
    "Bash:hamingja ledger check": 0.0,
    "Bash:hamingja ledger relevant": 0.0,
    "Bash:hamingja ledger add": 0.2,
    "Bash:hamingja ledger retire": 0.2,
    "Bash:hamingja ledger reverify": 1.0,
}
...
# ---------------------------------------------------------------------------
# State-dir helpers (mirrors state.py so both use HAMINGJA_STATE_DIR)
# ---------------------------------------------------------------------------

def _state_dir() -> Path:
    base = os.environ.get("HAMINGJA_STATE_DIR") or os.path.join(
        tempfile.gettempdir(), "hamingja"
    )
    p = Path(base)
...
def increment_and_check(
    ...
    try:
        ...
        if (stop_enabled and wc > hard_block_at and wc > approved_tc
            ...
            hard_msg = (
                f"[hamingja budget] Hard limit: {_fmt_calls(wc, tc)}/{hard_block_at} weighted calls{type_tag}.\n\n"
                f"Extend the budget:\n"
                f"  ! hamingja budget {session_id} add N\n\n"
                f"Or reset (clears all session state):\n"
                f"  ! hamingja budget {session_id} reset"
            )
        ...
        if is_subagent and sa > max_subagents and not subagent_approved:
            subagent_msg = (
                f"[hamingja budget] Subagent blocked: {sa} attempted, {max_subagents} approved.\n"
                f"Approve to continue (Claude will resume automatically):\n"
                f"  ! hamingja budget {session_id} subagent"
            )
            print(f"\n{subagent_msg}\n", file=sys.stderr, flush=True)
        ...
        if wc > approved_tc:
            ...
            if relief is not None:
                ...
                return BudgetVerdict(
                    NUDGE,
                    f"[hamingja budget] Checkpoint deferred at {_fmt_calls(wc, tc)} weighted "
            ...
            if not checkpoint_denies:
                ...
                return BudgetVerdict(
                    NUDGE,
                    f"[hamingja budget] Checkpoint advised: {_fmt_calls(wc, tc)}/{approved_tc} "
            ...
            if sa_enabled and sa_remaining > 0:
                return BudgetVerdict(
                    BLOCK,
                    f"[hamingja budget] Checkpoint: {_fmt_calls(wc, tc)}/{approved_tc} weighted calls used{type_tag}.\n\n"
                    f"Self-approve to continue. Run this as a Bash tool call, then retry:\n"
                    f"  hamingja budget {session_id} add N --self\n"
        ...
        if ctx_pct is not None and ctx_thresh > 0 and ctx_pct >= ctx_thresh:
            return BudgetVerdict(
                NUDGE,
                f"[hamingja budget] Context ~{ctx_pct:.0f}% full{type_tag}. A full "
        ...
        if is_large_read and lr > max_large_reads:
            return BudgetVerdict(
                NUDGE,
                f"[hamingja budget] Large unscoped reads: {lr} (soft limit {max_large_reads}). "
        ...
        if wc > nudge_at:
            return BudgetVerdict(
                NUDGE,
                f"[hamingja budget] {_fmt_calls(wc, tc)} weighted calls used{type_tag}. "
