...
def build_code_atlas(
    ...
    *,
    ...
    for path in _iter_source_files(root_path, glob):
        ...
        if not entries:
            continue
        out.append(AtlasFile(
            path=path.resolve(), line_count=line_count, entries=entries,
            generated=_is_generated(path, lines),
        ))
    ...
    for path in _iter_source_files(root_path, glob):
        ...
        out.append(
            HealthFile(
                ...
                suggestions=([] if _is_generated(path, lines) else _split_suggestions(
                    entries, max_suggestions, path.suffix.lower()
                )),
                generated=_is_generated(path, lines),
            )
        )
...
def format_repo_health(health: list[HealthFile], root: Path | str = ".") -> str:
    ...
    for item in health:
        ...
        if item.generated:
            lines.append("  generated artifact; source split advice suppressed")
        if item.suggestions:
...
def _split_suggestions(
    entries: list[AtlasEntry], max_suggestions: int, suffix: str = ".py"
) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    for entry in entries:
        if entry.depth > 1:
            continue
        name = _module_name(entry.name, suffix)
...
def _module_name(name: str, suffix: str = ".py") -> str:
    cleaned = re.sub(r"[^A-Za-z0-9]+", "_", name).strip("_").lower()
    if not cleaned:
        return ""
    extension = suffix if suffix in _SOURCE_EXTENSIONS else ".txt"
    if not cleaned.endswith(extension):
        cleaned += extension
    return cleaned
...
def _is_generated(path: Path, lines: list[str]) -> bool:
    try:
        ...
        if any(marker in name for marker in (".generated.", ".gen.", "_generated.")):
            return True
        header = "\n".join(lines[:8]).lower()
        return any(marker in header for marker in (
            "do not edit", "auto-generated", "autogenerated", "generated file",
        ))
    except Exception:
        return False
