...
1. Choose the version and update `hamingja.__version__` plus the changelog
   heading/date.
...
6. With temporary `CLAUDE_SETTINGS` and `CODEX_HOOKS` paths, exercise fresh
   ...
   hamingja paths, and uninstall while preserving unrelated hooks.
7. Verify README links, package metadata, Python requirement, and the installed
   `hamingja init --dry-run` output. Confirm the documented pipx install,
